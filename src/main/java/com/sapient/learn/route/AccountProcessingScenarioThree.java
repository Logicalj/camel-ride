...
@Configuration
public class AccountProcessingScenarioThree extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		.filter()
		  .ognl("request.body.entityType == 'Account'") // - Filter Message Content
		...
		.to("seda:publishingQueue-sf"); // ----------------- Publish to SEDA Queue 
		
		from("seda:publishingQueue-sf?concurrentConsumers=10") // SEDA consumer (10 consumers working independently)
		.process(new Processor() {
			@Override
			public void process(Exchange exchange) throws Exception {
				log.info("Processing for Entity id: "+ ( (Entity) exchange.getIn()).getEntityId());				
			}
		});
	}
